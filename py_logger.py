...
class CvLogWidget(VisualLogWidget):
    ...
    def draw_text_line(self, line, canvas=None):
        ...
        self.cv2.putText(canvas, line.text, (int(line.absolute_position.x), int(
            line.absolute_position.y)), self.cv2.FONT_HERSHEY_SIMPLEX, line.font_size, line.color, line.thickness, self.cv2.LINE_AA)
...
class PyGameLogWidget(VisualLogWidget):
    ...
    def draw_text_line(self, line, canvas=None):
        ...
        canvas.blit(line.font.render(line.text, True, line.color),
                    (int(line.absolute_position.x), int(line.absolute_position.y)))
